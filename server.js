    ...
    if (req.method === 'POST' && req.url === '/heartbeat') {
        ...
        req.on('end', () => {
            try {
                ...
                if (!tabs[tabId]) tabs[tabId] = { count: 0, total: 0, site: site || 'bwanabet' };
                tabs[tabId].lastSeen = Date.now();
                tabs[tabId].site = site || 'bwanabet';
                res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    if (req.method === 'POST' && req.url === '/cashout') {
        ...
        req.on('end', () => {
            try {
                ...
                totalProfit += amount;
                cashouts.unshift({ tabId, amount, timestamp, site: site || 'bwanabet' });
                if (cashouts.length > 500) cashouts.pop();
                if (!tabs[tabId]) tabs[tabId] = { count: 0, total: 0, lastSeen: Date.now(), site: site || 'bwanabet' };
                tabs[tabId].lastSeen = Date.now();
                tabs[tabId].count++;
